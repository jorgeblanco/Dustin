...
namespace Dustin.Scripts.VR
{
	...
	public class HandVR : MonoBehaviour
	{
...
#if UNITY_EDITOR
		#region Debug
		Vector3 _hoveredClosestPoint;
		#endregion
#endif
		#region References
		private Interactable _hoveredInteractable = null;
		private Grabbable _attachedGrabbable = null;
		private HandVR _otherHandVR = null;
		#endregion
		...
		public bool Blocked { get { if (_blocked == null) _blocked = !Visible; return _blocked.Value; } }  //On late update we make it null so this is calculated one time per frame
		public Interactable HoveredInteractable
		{
			get { return _hoveredInteractable; }
			private set
			{
				if (value == _hoveredInteractable) return;
				_hoveredInteractable?.Unhover(this);
		...
		public Grabbable AttachedGrabbable
		{
			...
			set
			{
				...
				else
				{
					...
					CharacterVR.FreeCollisionIgnores(_attachedGrabbable);
			...
			get
			{
				if (!_otherHandVR)
				{
					for (int i = 0; i < CharacterVR.HandsVR.Length; i++)
					{
						...
						{
							_otherHandVR = CharacterVR.HandsVR[i];
							break;
						}
					}
				}
...
//#endif
			...
			for (int i = 0; i < results.Length; i++)
			{
				...
				if (grabbable)
				{
					...
					{
#if UNITY_EDITOR
						_hoveredClosestPoint = closestPoints[i];
#endif
						return grabbable;
					}
					continue;
		...
		private void OnDrawGizmos()
		{
			...
			if(HoveredInteractable) Debug.DrawLine(transform.position, _hoveredClosestPoint, Color.green);
		}
