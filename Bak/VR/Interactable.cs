...
namespace Dustin.Scripts.VR
{
	...
	public class Interactable : MonoBehaviour
	{
		...
		#region Fields
		private bool? _overlapping = null;
		...
		public bool Overlapping { get { if (_overlapping == null) _overlapping = IsOverlapping(); return _overlapping.Value; } }
			...
			for (int i = 0; i < Colliders.Length; i++)
			{
				...
				else
				{
					...
					if (capsuleCollider)
					{
						...
						var lossyScale = capsuleCollider.transform.lossyScale;
						...
						if (capsuleCollider.direction == 0)
						{
							...
							height = capsuleCollider.height * lossyScale.x;
							radius = capsuleCollider.radius * Mathf.Max(lossyScale.y, lossyScale.z);
						}
						else if (capsuleCollider.direction == 2)
						{
							direction = Vector3.forward;
							height = capsuleCollider.height * capsuleCollider.transform.lossyScale.z;
							radius = capsuleCollider.radius * Mathf.Max(lossyScale.x, lossyScale.y);
						}
